...
class IGenParsBuilder(metaclass=ABCMeta):
    """The Generic Parameters Builder Interface Class"""

    @abstractstaticmethod
    def get_pars(self):
        """Return the Paramters Object."""
    
    @abstractstaticmethod
    def set_pop_pars(self):
        """Set population parameters."""        
        
    @abstractstaticmethod
    def set_sim_pars(self):
    ...
    @abstractstaticmethod
    def set_rescale_pars(self):
        """Set rescale parameters."""
         
    @abstractstaticmethod
    def set_trans_pars(self):
        """Set trans parameters."""
         
    @abstractstaticmethod
    def set_multivar_pars(self):
        """Set multivariable parameters."""
         
    @abstractstaticmethod
    def set_immunity_pars(self):
        """Set immunity parameters."""
         
    @abstractstaticmethod
    def set_nab_efficacy_pars(self):
        """Set Nab efficacy parameters."""
         
    @abstractstaticmethod
    def set_varTrans_pars(self): 
        """Set variable trans parameters."""
        
    @abstractstaticmethod
    def set_duration_pars(self): 
        """Set duration parameters."""
        
    @abstractstaticmethod
    def set_severity_pars(self, prog_by_age):
        """Set severity parameters."""
        
    @abstractstaticmethod
    def set_protect_efficy_pars(self): 
        """Set protection efficacy parameters."""
        
    @abstractstaticmethod 
    def set_event_interv_pars(self):
        """Set event intervention parameters."""
        
    @abstractstaticmethod
    def set_health_sys_pars(self):
        """Set health system  parameters."""
    
    @abstractstaticmethod
    def update_gen_vacvar_pars(self):
        """Set health system  parameters."""
    
    @abstractstaticmethod
    def update_all_pars(self):
        """Set health system  parameters."""
    
    @abstractstaticmethod
    def reset_layer_pars(self):
         """Set health system  parameters."""
    @abstractstaticmethod
...
class GenParsBuilder(IGenParsBuilder):
    
    """The concrete generic paramaters builder"""
    
    __all__ = ['set_pars', 'reset_layer_pars', 'get_prognoses', 'get_variant_choices', 'get_vaccine_choices']
    ...
    def get_pars(self):
        """Get the result, i.e the pars object"""
        return self.pars
    
    def set_pop_pars(self) -> dict:
        '''
        ...
        self.pars['pop_size']     = 20e3     # Number of agents, i.e., people susceptible to SARS-CoV-2
        self.pars['pop_infected'] = 20       # Number of initial infections
        self.pars['pop_type']     = 'random' # What type of population data to use -- 'random' (fastest), 'synthpops' (best), 'hybrid' (compromise)
        self.pars['location']     = None     # What location to load data from -- default Seattle
        return self
    
    def set_sim_pars(self) -> dict:
        '''
        ...
        self.pars['start_day']  = '2020-03-01' # Start day of the simulation
        self.pars['end_day']    = None         # End day of the simulation
        self.pars['n_days']     = 60           # Number of days to run, if end_day isn't specified
        self.pars['rand_seed']  = 1            # Random seed, if None, don't reset
        self.pars['verbose']    = cvo.verbose  # Whether or not to display information during the run -- options are 0 (silent), 0.1 (some; default), 1  (default), 2 (everything)
        return self

    def set_rescale_pars(self) -> dict:
        '''
        ...
        self.pars['pop_scale']         = 1    # Factor by which to scale the population -- e.g. pop_scale=10 with pop_size=100e3 means a population of 1 million
        self.pars['scaled_pop']        = None # The total scaled population, i.e. the number of agents times the scale factor
        self.pars['rescale']           = True # Enable dynamic rescaling of the population -- starts with pop_scale=1 and scales up dynamically as the epidemic  grows
        self.pars['rescale_threshold'] = 0.05 # Fraction susceptible population that will trigger rescaling if rescaling
        self.pars['rescale_factor']    = 1.2  # Factor by which the population is rescaled on each step
        self.pars['frac_susceptible']  = 1.0  # What proportion of the population is susceptible to infection
        return self

    def set_network_pars(self) -> dict:
        '''
        ...
        self.pars['contacts']        = None  # The number of contacts per layer; set by reset_layer_pars() below
        self.pars['dynam_layer']     = None  # Which layers are dynamic; set by reset_layer_pars() below
        self.pars['beta_layer']      = None  # Transmissibility per layer; set by reset_layer_pars() below
        return self

    def set_trans_pars(self) -> dict:
        '''
        ...
        self.pars['beta_dist']       = dict(dist='neg_binomial', par1=1.0, par2=0.45, step=0.01) # Distribution to draw individual level transmissibility;   dispersion from https://www.researchsquare.com/article/rs-29548/v1
        self.pars['viral_dist']      = dict(frac_time=0.3, load_ratio=2, high_cap=4) # The time varying viral load (transmissibility); estimated from Lescure    2020, Lancet, https://doi.org/10.1016/S1473-3099(20)30200-0
        self.pars['beta'] = 0.016  # Beta per symptomatic contact; absolute value, calibrated
        self.pars['asymp_factor']    = 1.0  # Multiply beta by this factor for asymptomatic cases; no statistically significant difference in transmissibility:  https://www.sciencedirect.com/science/article/pii/S1201971220302502
        return self

    def set_multivar_pars(self) -> dict:
        '''
        ...
        self.pars['n_imports'] = 0 # Average daily number of imported cases (actual number is drawn from Poisson distribution)
        self.pars['n_variants'] = 1 # The number of variants circulating in the population
        return self

    def set_immunity_pars(self) -> dict:
        '''
        ...
        self.pars['use_waning']      = False # Whether to use dynamically calculated immunity
        self.pars['nab_init']        = dict(dist='normal', par1=0, par2=2)  # Parameters for the distribution of the initial level of log2(nab) following    natural infection, taken from fig1b of https://doi.org/10.1101/2021.03.09.21252641
        self.pars['nab_decay']       = dict(form='nab_growth_decay', growth_time=22, decay_rate1=np.log(2)/100, decay_time1=250, decay_rate2=np.log(2)/3650,     decay_time2=365)
        self.pars['nab_kin']         = None # Constructed during sim initialization using the nab_decay parameters
        self.pars['nab_boost']       = 1.5 # Multiplicative factor applied to a person's nab levels if they get reinfected. # TODO: add source
        self.pars['nab_eff']         = dict(alpha_inf=3.5, beta_inf=1.219, alpha_symp_inf=-1.06, beta_symp_inf=0.867, alpha_sev_symp=0.268, beta_sev_symp=3.4) 
        return self

    def set_nab_efficacy_pars(self) -> dict:
        '''
        ...
        self.pars['rel_imm_symp']    = dict(asymp=0.85, mild=1, severe=1.5) # Relative immunity from natural infection varies by symptoms
        self.pars['immunity']        = None  # Matrix of immunity and cross-immunity factors, set by init_immunity() in immunity.py
        return self

    def set_varTrans_pars(self) -> dict:
        '''
        ...
        self.pars['rel_beta']        = 1.0 # Relative transmissibility varies by variant
        self.pars['rel_imm_variant']  = 1.0 # Relative own-immmunity varies by variant
        return self

    def set_duration_pars(self) -> dict:
        '''
    ...
    def set_severity_pars(self, prog_by_age=True) -> dict:
        '''
        ...
        self.pars['rel_symp_prob']   = 1.0  # Scale factor for proportion of symptomatic cases
        self.pars['rel_severe_prob'] = 1.0  # Scale factor for proportion of symptomatic cases that become severe
        self.pars['rel_crit_prob']   = 1.0  # Scale factor for proportion of severe cases that become critical
        self.pars['rel_death_prob']  = 1.0  # Scale factor for proportion of critical cases that result in death
        self.pars['prog_by_age']     = prog_by_age # Whether to set disease progression based on the person's age
        self.pars['prognoses']       = None # The actual arrays of prognoses by age; this is populated later
        return self

    def set_protect_efficy_pars(self) -> dict:
        '''
        ...
        self.pars['iso_factor']   = None # Multiply beta by this factor for diagnosed cases to represent isolation; set by reset_layer_pars() below
        self.pars['quar_factor']  = None # Quarantine multiplier on transmissibility and susceptibility; set by reset_layer_pars() below
        self.pars['quar_period']  = 14   # Number of days to quarantine for; assumption based on standard policies
        return self

    def set_event_interv_pars(self) -> dict:
        '''
        ...
        self.pars['interventions'] = []   # The interventions present in this simulation; populated by the user
        self.pars['analyzers']     = []   # Custom analysis functions; populated by the user
        self.pars['timelimit']     = None # Time limit for the simulation (seconds)
        self.pars['stopping_func'] = None # A function to call to stop the sim partway through
        return self

    def set_health_sys_pars(self) -> dict:
        '''
        ...
        self.pars['n_beds_hosp']    = None # The number of hospital (adult acute care) beds available for severely ill patients (default is no constraint)
        self.pars['n_beds_icu']     = None # The number of ICU beds available for critically ill patients (default is no constraint)
        self.pars['no_hosp_factor'] = 2.0  # Multiplier for how much more likely severely ill people are to become critical if no hospital beds are available
        self.pars['no_icu_factor']  = 2.0  # Multiplier for how much more likely critically ill people are to die if no ICU beds are available
        return self
    
    def update_gen_vacvar_pars(self):
        # Handle vaccine and variant parameters
        self.pars['vaccine_pars']  = {} # Vaccines that are being used; populated during initialization
        self.pars['vaccine_map']   = {} #Reverse mapping from number to vaccine key
        self.pars['variants']      = [] # Additional variants of the virus; populated by the user, see immunity.py
        self.pars['variant_map']   = {0:'wild'} # Reverse mapping from number to variant key
        self.pars['variant_pars']  = dict(wild={}) # Populated just below
        for sp in cvd.variant_pars:
            if sp in self.pars.keys():
                self.pars['variant_pars']['wild'][sp] = self.pars[sp]
    
    def update_all_pars(self, set_prognoses, version, **kwargs):
        # Update with any supplied parameter values and generate things that need to be generated
        self.pars.update(kwargs)
        self.reset_layer_pars()
        if set_prognoses: # If not set here, gets set when the population is initialized
            self.pars['prognoses'] = Prognoses.get_prognoses(self.pars['prog_by_age'], version=version) # Default to age-specific prognoses
        # If version is specified, load old parameters
        if version is not None:
            version_pars = cvm.get_version_pars(version, verbose=self.pars['verbose'])
            for key in self.pars.keys(): # Only loop over keys that have been populated
                if key in version_pars: # Only replace keys that exist in the old version
                    self.pars[key] = version_pars[key]
            # Handle code change migration
            if sc.compareversions(version, '2.1.0') == -1 and 'migrate_lognormal' not in self.pars:
                cvm.migrate_lognormal(self.pars, verbose=self.pars['verbose'])
    ...
    def reset_layer_pars(self, layer_keys=None, force=False): 
        ...
        try:
            defaults = layer_defaults[self.pars['pop_type']]
        except Exception as E:
            errormsg = f'Cannot load defaults for population type "{self.pars["pop_type"]}": must be hybrid, random, or synthpops'
        ...
        for pkey in self.layer_pars:
            ...
            if force:
                ...
                par_dict = sc.mergedicts(defaults[pkey], self.pars.get(pkey, None)) # Use user-supplied parameters if available, else default
...
class GenPars:
    ...
    def __init__(self, set_prognoses=True, prog_by_age=True, version = None, **kwargs) -> dict:
        ...
        self.pars = self.pars.set_sim_pars() # add simulation pars
        self.pars = self.pars.set_network_pars() # add network pars
        self.pars = self.pars.set_rescale_pars() # add rescaling pars
        self.pars = self.pars.set_trans_pars() # add disease transmission parameters
        self.pars = self.pars.set_multivar_pars() # add pars that control settings and defaults for multi-variant runs 
        self.pars = self.pars.set_immunity_pars() # add the immunity pars
        self.pars = self.pars.set_nab_efficacy_pars() # add pars which map nabs to efficacy.
        self.pars = self.pars.set_varTrans_pars() # add variant-specifc disease transimssion pars
        self.pars = self.pars.set_duration_pars() # add duration pars for disease progression and recovery 
        self.pars = self.pars.set_severity_pars(prog_by_age) # add severity pars 
        self.pars = self.pars.set_protect_efficy_pars() # add efficacy of protection measures pars
        self.pars = self.pars.set_event_interv_pars() # add event and intervention pars
        self.pars = self.pars.set_health_sys_pars() # add health system pars
        self.pars = self.update_gen_vacvar_pars() # Handle vaccine and variant parameters
        self.pars = self.update_all_pars(set_prognoses,version, kwargs) # Update with any supplied parameter values and generate things that need to be generated
        return self.pars
class PrognosesBuilder(IGenParsBuilder):
    ...
    def get_prognoses(by_age=True, version=None):
        ...
        prognoses = PrognosesBuilder.relative_prognoses(prognoses) # Convert to conditional probabilities
...
#%% Variant parameters and functions
class VariantBuilder(IGenParsBuilder):
...
class GenParsDirector:
    """The director for GenPars which helps build diffrent representations."""
    @staticmethod
    def construct():
        return GenParsBuilder().set_pop_pars().set_sim_pars().set_network_pars().set_rescale_pars().set_trans_pars().set_multivar_pars().set_immunity_pars().set_nab_efficacy_pars().set_varTrans_pars().set_duration_pars().set_severity_pars(prog_by_age=True).set_protect_efficy_pars().set_event_interv_pars().set_health_sys_pars().update_gen_vacvar_pars().update_all_pars(set_prognoses=True,version=None).get_pars()
class PrognosesParsDirector:
    """The director for Prognoses Parameters which helps build diffrent representations."""
    @staticmethod
    def construct():
        return GenParsBuilder().get_prognoses()

class VariantParsDirector:
    """The director for Variant Parameters which helps build diffrent representations."""
    @staticmethod
    def construct():
        return GenParsBuilder().get_variant_pars(default=False)

class VaccineVariantParsDirector:
    """The director for Vaccine variant Parameters which helps build diffrent representations."""
    @staticmethod
    def construct():
        return GenParsBuilder().get_vaccine_variant_pars()
    
class VaccineDoseParsDirector:
    """The director for Vaccine dose Parameters which helps build diffrent representations."""
    @staticmethod
    def construct():
        return GenParsBuilder().get_vaccine_dose_pars(default=False)
